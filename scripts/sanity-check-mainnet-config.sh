...
for key in RPC_URL PRIVATE_SUPPLY_ADAPTER SUPPLY_TOKEN AAVE_POOL VAULT_FACTORY RAILGUN_CALLBACK_SENDER RAILGUN_SHIELD; do
  require_env "${key}"
done

for key in PRIVATE_SUPPLY_ADAPTER SUPPLY_TOKEN AAVE_POOL VAULT_FACTORY RAILGUN_CALLBACK_SENDER RAILGUN_SHIELD; do
...
ADAPTER_CALLBACK="$(cast call "${PRIVATE_SUPPLY_ADAPTER}" "railgunCallbackSender()(address)" --rpc-url "${RPC_URL}")"
ADAPTER_SHIELD="$(cast call "${PRIVATE_SUPPLY_ADAPTER}" "railgunShield()(address)" --rpc-url "${RPC_URL}")"
...
echo "Callback (env/adp):  ${RAILGUN_CALLBACK_SENDER} / ${ADAPTER_CALLBACK}"
echo "Shield (env/adp):    ${RAILGUN_SHIELD} / ${ADAPTER_SHIELD}"
...
if [[ "$(to_lower "${RAILGUN_CALLBACK_SENDER}")" != "$(to_lower "${ADAPTER_CALLBACK}")" ]]; then
  echo "Error: adapter railgunCallbackSender() mismatch." >&2
  exit 1
fi
if [[ "$(to_lower "${RAILGUN_SHIELD}")" != "$(to_lower "${ADAPTER_SHIELD}")" ]]; then
  echo "Error: adapter railgunShield() mismatch." >&2
  exit 1
fi
...
ARB_CALLBACK="0x5aD95C537b002770a39dea342c4bb2b68B1497aA"
ARB_SHIELD="0xFA7093CDD9EE6932B4eb2c9e1cde7CE00B1FA4b9"
...
if [[ "${CHAIN_ID}" == "42161" ]]; then
  ...
  if [[ "$(to_lower "${RAILGUN_CALLBACK_SENDER}")" != "$(to_lower "${ARB_CALLBACK}")" ]]; then
    echo "Warning: callback sender does not match Arbitrum Railgun relay-adapt." >&2
  fi
  if [[ "$(to_lower "${RAILGUN_SHIELD}")" != "$(to_lower "${ARB_SHIELD}")" ]]; then
    echo "Warning: shield address does not match Arbitrum Railgun proxy." >&2
  fi
fi
