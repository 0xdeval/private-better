...
SMOKE_STAKE_AMOUNT="${SMOKE_STAKE_AMOUNT:-20000000}" # 20 USDC
SMOKE_BORROW_AMOUNT="${SMOKE_BORROW_AMOUNT:-100000000000000}" # 0.0001 WETH
SMOKE_REPAY_AMOUNT="${SMOKE_REPAY_AMOUNT:-${SMOKE_BORROW_AMOUNT}}"
...
SMOKE_ZK_OWNER_LABEL="${SMOKE_ZK_OWNER_LABEL:-zk-owner-repay-smoke-$(date +%s)}"
SMOKE_WITHDRAW_SECRET_LABEL="${SMOKE_WITHDRAW_SECRET_LABEL:-smoke-$(date +%Y%m%d-%H%M%S)-repay}"
...
REQUEST_DATA="$(
  ...
  bun -e '
    ...
    const zkOwnerHash = keccak256(toUtf8Bytes(process.env.SMOKE_ZK_OWNER_LABEL ?? "zk-owner-repay-smoke"));
    const withdrawSecret = keccak256(toUtf8Bytes(process.env.SMOKE_WITHDRAW_SECRET_LABEL ?? "withdraw-secret-repay-smoke"));
...
AUTH_SECRET="$(
  ...
  bun -e 'import { ethers } from "ethers"; console.log(ethers.utils.keccak256(ethers.utils.toUtf8Bytes(process.env.SMOKE_WITHDRAW_SECRET_LABEL ?? "withdraw-secret-repay-smoke")));'
)"
NEXT_SECRET="$(
  SMOKE_WITHDRAW_SECRET_LABEL="${SMOKE_WITHDRAW_SECRET_LABEL}" \
  bun -e 'import { ethers } from "ethers"; console.log(ethers.utils.keccak256(ethers.utils.toUtf8Bytes((process.env.SMOKE_WITHDRAW_SECRET_LABEL ?? "withdraw-secret-repay-smoke") + "-next")));'
)"
NEXT_AUTH_HASH="$(
...
FINAL_SECRET="$(
  SMOKE_WITHDRAW_SECRET_LABEL="${SMOKE_WITHDRAW_SECRET_LABEL}" \
  bun -e 'import { ethers } from "ethers"; console.log(ethers.utils.keccak256(ethers.utils.toUtf8Bytes((process.env.SMOKE_WITHDRAW_SECRET_LABEL ?? "withdraw-secret-repay-smoke") + "-final")));'
)"
FINAL_AUTH_HASH="$(
  FINAL_SECRET="${FINAL_SECRET}" \
  bun -e 'import { ethers } from "ethers"; console.log(ethers.utils.keccak256(process.env.FINAL_SECRET ?? ""));'
)"
REPAY_CLEANUP_SECRET="$(
  SMOKE_WITHDRAW_SECRET_LABEL="${SMOKE_WITHDRAW_SECRET_LABEL}" \
  bun -e 'import { ethers } from "ethers"; console.log(ethers.utils.keccak256(ethers.utils.toUtf8Bytes((process.env.SMOKE_WITHDRAW_SECRET_LABEL ?? "withdraw-secret-repay-smoke") + "-repay-cleanup")));'
)"
WITHDRAW_CLEANUP_SECRET="$(
  SMOKE_WITHDRAW_SECRET_LABEL="${SMOKE_WITHDRAW_SECRET_LABEL}" \
  bun -e 'import { ethers } from "ethers"; console.log(ethers.utils.keccak256(ethers.utils.toUtf8Bytes((process.env.SMOKE_WITHDRAW_SECRET_LABEL ?? "withdraw-secret-repay-smoke") + "-withdraw-cleanup")));'
)"

echo "Starting PrivateSupplyAdapter repay smoke test"
...
echo "Repay amount (raw): ${SMOKE_REPAY_AMOUNT}"
echo "Cleanup mode: ${SMOKE_CLEANUP}"

if is_true "${SMOKE_SET_EXECUTOR_TO_DEPLOYER}"; then
  echo "Step 1/8: Setting executor to deployer..."
  cast send "${PRIVATE_SUPPLY_ADAPTER}" \
...
echo "Step 2/8: Funding adapter with collateral token..."
...
echo "Step 3/8: Opening collateral position..."
cast send "${PRIVATE_SUPPLY_ADAPTER}" \
  "onPrivateDeposit(address,uint256,bytes)" "${SUPPLY_TOKEN}" "${SMOKE_STAKE_AMOUNT}" "${REQUEST_DATA}" \
  --rpc-url "${RPC_URL}" --private-key "${DEPLOYER_PRIVATE_KEY}" >/dev/null

echo "Step 4/8: Borrowing..."
cast send "${PRIVATE_SUPPLY_ADAPTER}" \
  "borrowToRecipient(uint256,address,uint256,bytes32,bytes32,address)" "${POSITION_ID}" "${BORROW_TOKEN}" "${SMOKE_BORROW_AMOUNT}" "${AUTH_SECRET}" "${NEXT_AUTH_HASH}" "${DEPLOYER_ADDRESS}" \
  --rpc-url "${RPC_URL}" --private-key "${DEPLOYER_PRIVATE_KEY}" >/dev/null

echo "Step 5/8: Funding adapter with borrow token for repay..."
cast send "${BORROW_TOKEN}" \
  "transfer(address,uint256)" "${PRIVATE_SUPPLY_ADAPTER}" "${SMOKE_REPAY_AMOUNT}" \
  --rpc-url "${RPC_URL}" --private-key "${DEPLOYER_PRIVATE_KEY}" >/dev/null

echo "Step 6/8: Repaying..."
cast send "${PRIVATE_SUPPLY_ADAPTER}" \
  "repayFromPrivate(uint256,address,uint256,bytes32,bytes32)" "${POSITION_ID}" "${BORROW_TOKEN}" "${SMOKE_REPAY_AMOUNT}" "${NEXT_SECRET}" "${FINAL_AUTH_HASH}" \
  --rpc-url "${RPC_URL}" --private-key "${DEPLOYER_PRIVATE_KEY}" >/dev/null

echo "Step 7/8: Reading results..."
...
if is_true "${SMOKE_CLEANUP}"; then
  echo "Cleanup: repaying any remaining debt and withdrawing collateral..."
  RESERVE_DATA="$({
    cast call "${AAVE_POOL}" \
...
echo "Step 8/8: Restoring executor..."
restore_executor
...
echo
echo "Repay smoke test completed."
